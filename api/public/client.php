...
function login($token, $username, $password){
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.19.3.130:8080/login/" . $username . "/" . $password);
    curl_setopt( $ch, CURLOPT_HTTPHEADER, [
...
function getAllUsers($token) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.19.3.130:8080/person");
    curl_setopt( $ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        "Authorization: $token"
    ]);
...
function getUser($token, $id) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://172.19.3.130:8080/person/" . $id);
    curl_setopt( $ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        "Authorization: $token"
    ]);
